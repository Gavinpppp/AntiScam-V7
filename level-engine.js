  ...
  function renderDeepNarrative(scene) {
    ...
    function renderDeepChoices(cs, containerEl) {
      ...
      if (scene && scene.speaker === 'scammer') {
        const tFn = (typeof window.t === 'function') ? window.t : (k => {
          const map = {
            choice_free_other: '其他：自己輸入回應',
            choice_free_placeholder: '輸入你會點做…',
            choice_free_hint: '系統會根據你輸入嘅內容判斷係咪被騙',
            submit: '發送'
          };
          return map[k] || k;
        });
        ...
        const voiceBtnHTML = (typeof window.renderVoiceInputButtonHTML === 'function')
          ? window.renderVoiceInputButtonHTML('deepFreeVoiceBtn')
          : '';
        otherCard.innerHTML = `
          <button class="choice-btn choice-free-toggle" id="deepFreeToggle" type="button">
            <span class="choice-letter">✍️</span>
            <span class="choice-text">${tFn('choice_free_other')}</span>
            <span class="choice-expand-icon">▾</span>
          </button>
          <div class="choice-free-input-wrap" id="deepFreeInputWrap">
            <div class="choice-free-input-row">
              <input
                type="text"
                id="deepFreeInput"
                class="interactive-input choice-free-input"
                placeholder="${tFn('choice_free_placeholder')}"
                maxlength="200"
                autocomplete="off"
              >
              ${voiceBtnHTML}
              <button id="deepFreeSubmit" class="btn btn-submit choice-free-submit" type="button">
                ${tFn('submit')}
              </button>
            </div>
            <div class="choice-free-hint">${tFn('choice_free_hint')}</div>
          </div>
        `;
        containerEl.appendChild(otherCard);

        toggle = otherCard.querySelector('#deepFreeToggle');
        wrap = otherCard.querySelector('#deepFreeInputWrap');
        input = otherCard.querySelector('#deepFreeInput');
        submit = otherCard.querySelector('#deepFreeSubmit');
        voiceBtn = otherCard.querySelector('#deepFreeVoiceBtn');

        /* 綁定語音輸入 */
        if (voiceBtn && typeof window.setupVoiceInput === 'function') {
          window.setupVoiceInput(voiceBtn, input);
        }
      }
      ...
      containerEl.onclick = (e) => {
        /* 預設選項 */
        const choiceBtn = e.target.closest('.deep-choice-btn');
        if (choiceBtn) {
          if (choiceBtn.disabled) return;
          const idx = parseInt(choiceBtn.dataset.deepChoiceIndex, 10);
        ...
        if (submitBtn && input) {
          ...
          handleDeepFreeInput(cs, input, submitBtn);
          return;
        }
      };
      ...
      if (input) {
        input.onkeypress = (e) => {
          if (e.key === 'Enter') {
            e.preventDefault();
            handleDeepFreeInput(cs, input, submit);
          }
        };
      }
    ...
    /**
     * 處理 deep_narrative 場景中「其他：自由輸入」的文字提交
     * 同樣用 checkKeyword 偵測，並從現有 choices 推導 good / bad 路由
     */
    function handleDeepFreeInput(cs, inputEl, submitBtn) {
      if (!inputEl) return;
      ...
      const voiceBtn = document.getElementById('deepFreeVoiceBtn');
      ...
      const result = (typeof window.checkKeyword === 'function') ? window.checkKeyword(text) : 'neutral';

      const goodChoice = cs.find(c => c.feedbackType === 'good');
      const badChoice  = cs.find(c => c.feedbackType === 'bad');
      const midChoice  = cs.find(c => c.feedbackType === 'mid' || !c.feedbackType);

      let choice;
      if (result === 'good')      choice = goodChoice || midChoice || cs[0];
      else if (result === 'bad')  choice = badChoice || cs[0];
      else                         choice = midChoice || goodChoice || cs[0];

      if (choice) {
        /* 直接複用既有 handleConfigChoice，保持 effects / feedback / 跳轉邏輯一致 */
        handleConfigChoice(choice);
      }
    }
